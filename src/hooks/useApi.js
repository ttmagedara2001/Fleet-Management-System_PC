...
import api, { getStreamData, getStateDetails, updateState } from '../services/api';
...
export function useApi() {
    ...
    const getDeviceStreamData = useCallback(async (deviceId, startTime, endTime, pagination = 0, pageSize = 10) => {
        ...
        console.log('[API Hook] 📋 Params:', { deviceId, startTime, endTime, pagination, pageSize });
        ...
        try {
            ...
            console.error('[API Hook] ❌ getDeviceStreamData failed:', error.message);
            throw error;
        }
    }, []);
    ...
    const getTopicStreamData = useCallback(async (deviceId, topic, startTime, endTime, pagination = 0, pageSize = 10) => {
        ...
        try {
            ...
            console.error('[API Hook] ❌ getTopicStreamData failed:', error.message);
            throw error;
        }
    }, []);
    ...
    const getDeviceStateDetails = useCallback(async (deviceId) => {
        ...
        try {
            ...
            console.error('[API Hook] ❌ getDeviceStateDetails failed:', error.message);
            throw error;
        }
    }, []);
    ...
    const getTopicStateDetails = useCallback(async (deviceId, topic) => {
        ...
        try {
            ...
            console.error('[API Hook] ❌ getTopicStateDetails failed:', error.message);
            throw error;
        }
    }, []);
    ...
    const setThreshold = useCallback(async (deviceId, thresholdType, value) => {
        console.log('[API Hook] ⚙️ Setting threshold:', thresholdType, '=', value);
